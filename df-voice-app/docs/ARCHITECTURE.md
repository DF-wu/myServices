...
- `src/lib/workspace-storage.ts`: transcript, draft, and conversation workspace persistence.
...
The active transcript, raw ASR response, chat draft, and recent conversation messages are saved as a workspace snapshot so mobile app restarts and browser refreshes do not discard current work. Web stores that snapshot in `localStorage`; native stores it as an app-private document JSON file.
