...
- The active transcript, raw ASR response, draft, custom prompt/provider templates, and conversation are also stored locally so work survives app restarts. Custom provider templates may include provider credentials if the active setup has API keys. Web uses `localStorage`; native uses an app-private document JSON file. Use Settings -> Clear workspace before using shared devices or after handling sensitive transcripts.
