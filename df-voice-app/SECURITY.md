...
- The active transcript, raw ASR response, draft, and conversation are also stored locally so work survives app restarts. Web uses `localStorage`; native uses an app-private document JSON file. Clear sensitive workspace content before using shared devices.
