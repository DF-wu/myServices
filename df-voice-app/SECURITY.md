...
- The active transcript, raw ASR response, draft, custom prompt/provider templates, and conversation are also stored locally so work survives app restarts. Custom provider templates redact API keys and extra headers before they are written to workspace storage, then preserve current device credentials when applied. Web uses `localStorage`; native uses an app-private document JSON file. Use Settings -> Clear workspace before using shared devices or after handling sensitive transcripts. Destructive workspace and custom template actions require confirmation before local removal.
