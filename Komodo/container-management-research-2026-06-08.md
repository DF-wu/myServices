...
- Komodo deployment is prepared on LAN port `55003` and was test-started with Core, MongoDB, and Periphery.
- Komodo runtime is currently stopped; Portainer remains the only running management UI.
...
- Adds MongoDB.
- Local deployment now has gitignored `compose.env.local` secrets and completed first-run setup during the test start.
...
2. Start Komodo again only with explicit approval, then keep it LAN-only while migration is validated.
...
- `docker compose --env-file compose.env --env-file compose.env.local --profile agent up -d` was tested and starts Core, MongoDB, and Periphery.
- Komodo runtime was stopped after testing with `docker compose --env-file compose.env --env-file compose.env.local --profile agent down`; volumes were preserved.
