...
Panel URL when started: `http://192.168.10.13:55003`
...
Full-mode test status:

- `komodo-mongo`, `komodo-core`, and `komodo-periphery` were test-started.
- Periphery connected to Core as server `axolotl`.
- Periphery mounted `/var/run/docker.sock`, `/proc`, `/etc/komodo`, and `/config/keys`.
- Terminal features remain disabled by config
- Runtime was stopped after the test. Do not start full mode again without explicit approval.
...
## 2026-06-08 Full Start / Stop Record
...
```bash
docker compose --env-file compose.env --env-file compose.env.local --profile agent down
```

Result:

- No Komodo containers remain running.
- Komodo volumes were preserved; `down -v` was not used.
- Portainer remained running on `9000/9443`.
